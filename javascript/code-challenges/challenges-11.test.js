...
function transformToLis(obj){
  ...
  {
    let str = `<li>${arrkeys[i]}: ${arrvalue[i]}</li>`
    arr.push(str)
  }
  return arr
}
...
const count = (target, input) => {
  // Solution code here...
  let count =0;
  let countiner = input.filter(array=>
    {
      if(array.includes(target))
      {
        let cc= array.reduce((count,number)=>{
            if(number === target)
            count++
            return count
        },0)
...
const totalSum = (input) => {
  // Solution code here...
  let sum =0;
  let sumarr = input.map(arr=>{
    let sum1 = arr.reduce((sum,number)=>{
        sum=sum+number
        return sum
    },0)
    return sum1
  })
  sum = sumarr.reduce((sum2,tot)=>{
      sum2 = sum2+tot
      return sum2
  },0)
  return sum
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(obj=>{
      ...
      {
        strarr.push(Object.values(obj)[0])
      }
    })
    // console.log(strarr)
