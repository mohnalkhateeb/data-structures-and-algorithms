...
const createList = (availableItems) => {
  ...
  availableItems.forEach(val =>{
    if(val.available === true){itemavalabil.push(val.name)}
  })
  return itemavalabil
};
...
const fizzbuzz = (arr) => {
  ...
  let arr2 = [];
  arr.forEach(val=>{
    if(val % 3 === 0 && val % 5 === 0){arr2.push('Fizz Buzz')}
    else if (val % 3 === 0){arr2.push('Fizz')}
    else if (val % 5 === 0){arr2.push('Buzz')}
    else { arr2.push(val)}
  })
  return arr2
};
